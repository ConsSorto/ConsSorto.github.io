...
graph TD
    %% Tablas Dueños
    P([<b>PERSONA (D)</b><br/><u>id_persona (PK)</u><br/>nombre<br/>dni {Unique}])
    
    C([<b>CARRERA (D)</b><br/><u>id_carrera (PK)</u><br/>nombre_carrera])

    %% Tablas Pertenencia 1-1
    CE([<b>CUENTA_ESTUDIANTE (P)</b><br/><u>num_cuenta (PK)</u><br/>fecha_creacion<br/>id_persona (FK) {Unique, Not Null}])

    %% Tablas Pertenencia 1-M
    MC([<b>MEDIO_CONTACTO (P)</b><br/><u>id_contacto (PK)</u><br/>tipo (Correo/Cel)<br/>valor<br/>num_cuenta (FK)])

    %% Tabla Intermedia M-N
    EC([<b>ESTUDIANTE_CARRERA (P)</b><br/><u>id_inscripcion (PK)</u><br/>num_cuenta (FK)<br/>id_carrera (FK)])

    %% Conexiones con lógica Dueño-Pertenencia
    P -- "Propaga PK (1:1)" --> CE
    CE -- "Propaga PK (1:N)" --> MC
    CE -- "FK" --> EC
    C -- "FK" --> EC
    ...
    style CE fill:#f2f2f2,stroke:#333,stroke-width:2px
    style MC fill:#f2f2f2,stroke:#333,stroke-width:2px
    style EC fill:#e8e8e8,stroke:#333,stroke-width:2px
```
