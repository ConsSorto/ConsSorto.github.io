...
* **Variant**: Selecciona TypeScript + SWC (SWC es un compilador ultra rápido basado en Rust que reemplaza a Babel, ideal para proyectos pesados).
* **Linter**: Selecciona ESLint.

> [!NOTE]
> **Nota del Arquitecto (¿Por qué pnpm, TypeScript y ESLint?)**
> Primero, usamos `pnpm` en lugar del tradicional `npm` por pura optimización de hardware. `npm` descarga y duplica la pesada carpeta `node_modules` en cada proyecto que creas. `pnpm` utiliza enlaces duros (hard links) hacia un almacén global en tu sistema operativo, instalando las librerías una sola vez, lo que ahorra gigabytes de disco duro y reduce los tiempos de instalación a segundos.
> 
> Segundo, venimos de un backend donde C# no nos deja compilar si enviamos un String en lugar de un Int. En la web, JavaScript permite esos errores silenciosos. Al combinar TypeScript (tipado estricto) con ESLint (análisis estático o "corrector ortográfico" de código), blindamos nuestro Frontend. Si el backend exige un DTO con Id y Nombre, TypeScript nos obligará a enviar exactamente eso, garantizando la consistencia del contrato.
