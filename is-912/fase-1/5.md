...
graph TD
    Cliente["🍽️ Cliente <br/> (Petición HTTP)"]
    
    subgraph API ["Capa de Presentación"]
        Chef["👨🍳 El Chef <br/> (Controlador HTTP)"]
    end
    
    subgraph Contrato ["Capa de Aplicación"]
        Receta["📋 La Receta <br/> (Interfaz IHabitoRepository)"]
    end
    
    subgraph Implementacion ["Capa de Infraestructura"]
        Proveedor["🥩 Finca Proveedora <br/> (HabitoRepository)"]
        Gerente["👔 Gerente de Compras <br/> (Contenedor DI .NET)"]
    end

    Cliente -->|"1. Pide comida"| Chef
    Chef -->|"2. Mira lo que se pidió en"| Receta
    Proveedor -->|"3. Atiende y llena"| Receta
    Gerente -.->|"4. Define quién será la"| Proveedor

    style Cliente fill:#8E44AD,stroke:#9b59b6,stroke-width:2px,color:#fff
    style Chef fill:#2C3E50,stroke:#bdc3c7,stroke-width:2px,color:#fff
    style Receta fill:#F1C40F,stroke:#f39c12,stroke-width:4px,color:#000
